...
use crate::export::{Error, LabelSelector};
...
impl LabelSelector {
	...
	pub fn parse(input: &str) -> Result<LabelSelector, Error> {
		...
		for requirement in Self::split_requirements(input)? {
			let requirement = requirement.trim();
			if requirement.is_empty() {
				continue;
			}
			expressions.push(Self::parse_requirement(requirement)?);
		}

		let mut selector = Selector::default();
		selector.extend(expressions);
		Ok(LabelSelector(selector))
	}
	...
	fn parse_requirement(requirement: &str) -> Result<Expression, Error> {
		if let Some((key, values)) = Self::split_set_operator(requirement, "notin") {
			return Ok(Expression::NotIn(
				key,
				Self::parse_set(requirement, values)?,
			));
		}
		if let Some((key, values)) = Self::split_set_operator(requirement, "in") {
			return Ok(Expression::In(key, Self::parse_set(requirement, values)?));
		}
...
mod tests {
	...
	fn empty_selectors_match_everything() {
		...
		assert!(matches(",", &labels));
		assert!(LabelSelector::parse("").unwrap().0.selects_all());
	}

	#[test]
	fn rejects_malformed_selectors() {
		assert!(LabelSelector::parse("tier in test, prod").is_err());
		assert!(LabelSelector::parse("tier in (test").is_err());
		assert!(LabelSelector::parse("tier)").is_err());
		assert!(LabelSelector::parse("not a selector").is_err());
	}
}
