...
impl<T> DebugRefCell<T> {
    #[inline]
    pub const fn new(value: T) -> DebugRefCell<T> { DebugRefCell(RefCell::new(value)) }
}

#[cfg(not(debug_assertions))]
impl<T> DebugRefCell<T> {
    #[inline]
    pub const fn new(value: T) -> DebugRefCell<T> { DebugRefCell(UnsafeCell::new(value)) }
}
...
impl<T: ?Sized> DebugRefCell<T> {
    #[inline]
    pub fn borrow<'a>(&'a self) -> DebugRef<'a, T> { DebugRef(self.0.borrow()) }

    #[inline]
    pub fn borrow_mut<'a>(&'a self) -> DebugRefMut<'a, T> { DebugRefMut(self.0.borrow_mut()) }
