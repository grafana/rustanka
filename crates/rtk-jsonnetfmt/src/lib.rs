...
//! 0, 1, 2a and 2b of `docs/rtk-fmt-plan.md` are done: the lexer, the AST, the
//! parser, the unparser, the [`pass`] traversal and three of the twelve passes.
//! The other nine are no-ops, so a file that needs one of them comes back
//! unformatted; `quarantine.toml` names the fixtures that leaves failing and
...
/// # Current behaviour
///
/// `FormatNode`'s pipeline, with the nine passes that have not landed yet
...
/// | 11 | `EnforceStringStyle` | Phase 2c |
