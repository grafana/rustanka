...
impl ImportResolver for FileImportResolver {
	fn resolve_from(&self, from: &SourcePath, path: &dyn AsPathLike) -> Result<SourcePath> {
		...
		let mut direct = if let Some(f) = from.downcast_ref::<SourceFile>() {
			...
			// Also try library paths (e.g. when evaluating a snippet that imports the entrypoint)
		...
		if let ResolvePath::Str(s) = path {
			if s.starts_with("../") {
				let stripped_path = s.trim_start_matches("../");
				for library_path in &self.library_paths {
					let mut cloned = library_path.clone();
					cloned.push(ResolvePath::Str(stripped_path));
					if let Some(cloned) = check_path(&cloned)? {
						return Ok(cloned);
					}
				}
