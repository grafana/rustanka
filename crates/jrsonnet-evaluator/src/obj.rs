...
impl ObjValue {
	...
	fn get_idx(&self, key: IStr, core: CoreIdx) -> Result<Option<Val>> {
			...
			match cache.entry(cache_key.clone()) {
				Entry::Occupied(v) => match v.get() {
					...
					CacheValue::Pending => {
						...
						// Assertion is reading this field while it is still being computed.
						// Return the value from the previous layers (super) so evaluation can
						// proceed; matches go-jsonnet/tk behavior (e.g. super_assertion_recursion).
						// Only do this when asking for the full object (core.idx == len); for
						// intermediate layers we must compute normally or we feed None incorrectly.
						is_in_assertion() && core.idx == self.0.cores.len() && core.idx > 1
					}
				},
				Entry::Vacant(v) => {
		...
		if use_super_while_asserting {
			return self.get_idx(key, CoreIdx { idx: core.idx - 1 });
		}
