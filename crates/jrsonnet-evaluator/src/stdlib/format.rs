...
pub fn render_float(
	...
	if precision == 0 {
		...
		// render_integer expects non-negative value; sign is passed via neg flag.
		#[allow(clippy::bool_to_int_with_if)]
		let dot_size = if ensure_pt { 1 } else { 0 };
		padding = padding.saturating_sub(dot_size);
		render_decimal(out, n < 0.0, n.round().abs(), padding, 0, blank, sign);
		if ensure_pt {
			out.push('.');
		}
		return;
