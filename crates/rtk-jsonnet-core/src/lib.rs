...
pub use crate::native::{Arguments, Function, InfallibleArguments, Value};
...
pub trait Evaluator: Sized {
	...
	fn with_external_variable<'a, 'b, 'c, K, V>(
		&'a mut self,
		key: &'b K,
		value: &'c V,
	) -> &'a mut Self
	...
	fn with_native_function<'a, 'b, 'c, K, F>(
		&'a mut self,
		key: &'b K,
		func: &'c F,
	) -> &'a mut Self
	...
	fn with_top_level_argument<'a, 'b, 'c, K, V>(
		&'a mut self,
		key: &'b K,
		value: &'c V,
	) -> &'a mut Self
	...
	fn with_top_level_code<'a, 'b, 'c, K, V>(
		&'a mut self,
		key: &'b K,
		value: &'c V,
	) -> &'a mut Self
