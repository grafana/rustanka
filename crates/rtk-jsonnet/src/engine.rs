...
use rtk_jsonnet_core::{Evaluator as _, Function, FlagsExt, Implementation};
use rtk_spec::{DeepMerge};
use rtk_spec::canonical::{JsonnetImplementation, Rc};
use rtk_jsonnet_jrsonnet::{
    Error as JrsonnetError,
    Evaluator as JrsonnetEvaluator,
    Flag as JrsonnetFlag,
...
impl Evaluator {
	...
	pub fn with_native_function<'a, E, F>(&mut self, key: &'a str, function: F) -> Result<&mut Self, Error>
    where
...
impl Evaluator {
    fn populate_evaluator(&mut self, implementation: JsonnetImplementation) -> Result<&mut ImplementationEvaluator, Error> {
        let mut implementations = self.engine.implementations
            .write()
        ...
        let evaluator = match (implementation, &*implementations) {
            (JsonnetImplementation::Jrsonnet, Implementations { jrsonnet: Some(jrsonnet), .. }) =>
                self.evaluator.insert(ImplementationEvaluator::Jrsonnet(jrsonnet.create_evaluator())),
            _ => {
        ...
        //call_implementation_evaluator_method!(@with: evaluator, with_native_function, HELM GOES HERE);
...
impl Implementations {
    pub fn maybe_init_implementation(&mut self, implementation: JsonnetImplementation) -> Result<(), Error> {
