...
local scaledObjectQuery = |||
  (
    100 - (
      min(
        rate(container_cpu_usage_seconds_total{cluster=~"prod-us-central-0", namespace=~"loki-prod", container="partition-ingester", pod=~"partition-ingester-a.*"}[1m])
      ) / on(pod)
      max(
        rate(container_cpu_usage_seconds_total{cluster=~"prod-us-central-0", namespace=~"loki-prod", container="partition-ingester", pod=~"partition-ingester-a.*"}[1m])
      )[15m:]
...
local inlinePromqlQuery = '(100 - (min(rate(container_cpu_usage_seconds_total{cluster=~"prod-us-central-0", namespace=~"loki-prod", container="partition-ingester", pod=~"partition-ingester-a.*"}[1m])) / on(pod) max(rate(container_cpu_usage_seconds_total{cluster=~"prod-us-central-0", namespace=~"loki-prod", container="partition-ingester", pod=~"partition-ingester-a.*"}[1m]))[15m:])) and on() (up{job=~"loki-prod/rollout-operator"} > 0)) * 100';
        ...
        spec: {
          containers: [{
            ...
            args: [
              ...
              '-backend.endpoints=http://query-frontend-1.loki.svc.cluster.local:3100,http://query-frontend-2.loki.svc.cluster.local:3100',
              '-backend.preferred=http://query-frontend-1.loki.svc.cluster.local:3100',
