...
local partitionIngesterQuery = |||
  (
    100 - (
      min(rate(container_cpu_usage_seconds_total{cluster=~"prod-eu-central-0", namespace=~"loki-prod-039", container="partition-ingester", pod=~"partition-ingester-a.*"}[1m]))[15m:]
    )
    and on()
    (
      ...
      template: {
        ...
        spec: {
          containers: [{
            ...
            args: [
              '-backend.endpoints=http://query-frontend-1.loki.svc.cluster.local:3100,http://query-frontend-2.loki.svc.cluster.local:3100,http://query-frontend-3.loki.svc.cluster.local:3100',
