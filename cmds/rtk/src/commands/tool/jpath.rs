...
use rtk_jsonnet::jpath::{self, JPath};
...
fn format_debug(result: &JPath) -> String {
	let jsonnet_path = result.jsonnet_path();
	format!(
		"main: {}\nrootDir: {}\nbaseDir: {}\njpath: {}\n",
		result.entrypoint.display(),
		result.root_directory.display(),
		result.base_directory.display(),
...
mod tests {
	...
	fn test_normalize_path_does_not_escape_unix_root() {
		assert_eq!(
			jpath::normalize_path(Path::new("/foo/../../../etc")),
			PathBuf::from("/etc")
		);
		assert_eq!(
			jpath::normalize_path(Path::new("foo/..")),
			PathBuf::from(".")
		);
	}
	...
	fn test_dirs_no_base_empty() {
		let err = JPath::resolve(testdata("noBase/environments/empty")).unwrap_err();
		assert!(err.to_string().contains("could not find environment base"));
	}

	#[test]
	fn test_dirs_no_base_filename() {
		let err = JPath::resolve(testdata("noBase/environments/filename")).unwrap_err();
		assert!(err.to_string().contains("could not find environment base"));
	}

	#[test]
	fn test_dirs_no_base_no_main() {
		let err = JPath::resolve(testdata("noBase/environments/noMain")).unwrap_err();
		assert!(err.to_string().contains("could not find environment base"));
	}
	...
	fn test_dirs_no_root() {
		...
		std::fs::create_dir_all(temp.path().join("environments/default")).unwrap();
		std::fs::write(temp.path().join("environments/default/main.jsonnet"), "{}").unwrap();
		let err = JPath::resolve(temp.path().join("environments/default")).unwrap_err();
		assert!(err.to_string().contains("could not find project root"));
	}
	...
	fn test_dirs_valid() {
		...
		let result = JPath::resolve(data.join("environments/default")).unwrap();
		assert_eq!(result.root_directory, data);
		assert_eq!(result.base_directory, data.join("environments/default"));
	}

	#[test]
	fn test_dirs_valid_nested() {
		let data = testdata("valid");
		let result = JPath::resolve(data.join("environments/default/nestedDir")).unwrap();
		assert_eq!(result.root_directory, data);
		assert_eq!(result.base_directory, data.join("environments/default"));
	}
	...
	fn test_dirs_valid_parent_from_nested() {
		let data = testdata("valid");
		let result =
			JPath::resolve(data.join("environments/default/nestedDir").join("..")).unwrap();
		assert_eq!(result.root_directory, data);
		assert_eq!(result.base_directory, data.join("environments/default"));
	}
	...
	fn test_find_root_pointer_jsonnetfile() {
		let temp = tempfile::TempDir::new().unwrap();
		std::fs::write(temp.path().join("jsonnetfile.json"), "{}").unwrap();
		std::fs::create_dir_all(temp.path().join("environments/default")).unwrap();
		std::fs::write(temp.path().join("environments/default/main.jsonnet"), "{}").unwrap();

		let result = JPath::resolve(temp.path().join("environments/default")).unwrap();
		assert_eq!(result.root_directory, temp.path());
		assert_eq!(
			result.base_directory,
			temp.path().join("environments/default")
		);
	}
	...
	fn test_find_root_pointer_tkrc() {
		let temp = tempfile::TempDir::new().unwrap();
		std::fs::write(temp.path().join("tkrc.yaml"), "").unwrap();
		std::fs::create_dir_all(temp.path().join("environments/default")).unwrap();
		std::fs::write(temp.path().join("environments/default/main.jsonnet"), "{}").unwrap();
